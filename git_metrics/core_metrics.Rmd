...
get_repo_collaborators <- function(repo) {
  ...
  tbl_repo_collaborators <- lst_collaborators %>% 
    map_dfr(function(x) {
      tibble(
        repo = repo
        , user = x$login
...
tbl_collaborators <- tbl_repo$name %>% 
  map_dfr(get_repo_collaborators)
```

```{r}
tbl_collaborators %>% 
  filter(!user %in% c('aadler', 'ResearchActuary')) %>% 
